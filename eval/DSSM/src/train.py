...
from model_utils import create_cnn2seq,create_seq2seq,create_ASM
from config import cnn2seq_config,seq2seq_config,ASM_config
import data_utils
...
def train(create_func,conf):
    ...
    with open(conf.model_dir+'/results.txt','a') as f:
        ...
        train_neg = read_data('/home/zyma/work/projects/cnn2seq/data_bpe/train_neg.ids60000.in',conf.max_train_data_size)
        # A bucket scale is a list of increasing numbers from 0 to 1 that we'll use
        # to select a bucket. Length of [scale[i], scale[i+1]] is proportional to
        # the size if i-th training bucket, as used later
        ...
        while model.global_step.eval(session=sess)<max_global_step:
          # Get a batch and make a step.
          start_time = time.time()
          pos_enc,pos_dec = model.get_batch(train_set,model.batch_size//2)
          neg_enc,neg_dec = model.get_batch(train_neg,model.batch_size//2)
          input_enc = []
          input_dec = []

          for ind,ele in enumerate(pos_enc):
            input_enc.append(np.concatenate([ele,neg_enc[ind]],axis=0))
          for ind,ele in enumerate(pos_dec):
            input_dec.append(np.concatenate([ele,neg_dec[ind]],axis=0))
          labels = model.batch_size//2*[1]+model.batch_size//2*[0]
          time_ = time.time()-start_time
          #print(time_)
          step_loss,_ = model.step(sess, input_enc,input_dec,labels)
          #print(time.time()-start_time-time_)
          step_time += (time.time() - start_time) / conf.steps_per_checkpoint
          loss += step_loss / conf.steps_per_checkpoint
          current_step += 1
          #print('loss:%f'%loss)
          # Once in a while, we save checkpoint, print statistics, and run evals.
          if current_step % conf.steps_per_checkpoint == 0:
            # Print statistics for the previous epoch.
            #perplexity = math.exp(loss) if loss < 300 else float('inf')
            step_result = "Global Step %d LR %.4f Step Time %.2f Test loss %f\n" %(model.global_step.eval(session=sess),
                          model.learning_rate.eval(session=sess), step_time, loss)
            #model.cnn_encoder_g.eval(session=sess),model.rnn_encoder_g.eval(session=sess))
            f.write(step_result)
            print(step_result.strip())
            # Decrease learning rate if no improvement was seen over last 3 times.
            if len(previous_losses) > 2 and loss > max(previous_losses[-3:]):
              sess.run(model.learning_rate_decay_op)
            ...
            step_time, loss = 0.0,0.0
            #eval loss
            input_enc,input_dec = model.get_batch(dev_set,model.batch_size)
            eval_loss,eval_predict = model.step(sess,input_enc,input_dec,model.batch_size*[1],training=False)
...
if __name__ == '__main__':
  arg = sys.argv[1]
  if arg == 'cnn2seq':
    train(create_cnn2seq,cnn2seq_config())
  if arg == 'seq2seq':
    train(create_seq2seq,seq2seq_config())  
  if arg == 'ASM':
    train(create_ASM,ASM_config())
