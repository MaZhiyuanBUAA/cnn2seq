...
from model import cnn2seq,seq2seq,AS_model
...
def create_cnn2seq(session,conf,forward_only=False):
  model = cnn2seq(conf.input_len,
                            ...
                            with_rnn = conf.with_rnn,
                            position = conf.position,
                            Filter = conf.Filter,
                            name='cnn2seq')
  ckpt = tf.train.get_checkpoint_state(conf.model_dir)
  if ckpt:
    print("Reading cnn2seq model from %s"%ckpt.model_checkpoint_path)
...
def create_seq2seq(session,conf,forward_only=False):
  ...
  ckpt = tf.train.get_checkpoint_state(conf.model_dir)
  if ckpt:
    print("Reading seq2seq model from %s"%ckpt.model_checkpoint_path)
    model.saver.restore(session,ckpt.model_checkpoint_path)
  else:
    print("Created model with fresh params.")
    session.run(tf.global_variables_initializer())
    #session.run(model.assign_params)
  return model
def create_ASM(session,conf,forward_only=False):
  model = AS_model(conf.input_len,
                            conf.output_len,
                            conf.size,
                            conf.num_layers,
                            conf.learning_rate,
  ...
  ckpt = tf.train.get_checkpoint_state(conf.model_dir)
  if ckpt:
    print("Reading answer selection model from %s"%ckpt.model_checkpoint_path)
    session.run(tf.global_variables_initializer())
    model.saver.restore(session,ckpt.model_checkpoint_path)
  else:
    print("Created model with fresh params.")
...
def get_predicted_sentence(input_sentence, vocab, rev_vocab, model, sess):
    ...
    input_data = model.get_batch(feed_data)
    # Get output logits for the sentence.
    _,output_logits = model.step(sess, *input_data,forward_only=True)
    outputs = []
